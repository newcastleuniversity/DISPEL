...
def data_trace_example(reading_example):
    ...
    feature2 = FeatureValue(ValueDefinition("feat2", "feat"), 5)
    reading.set(feature2)

    feature3 = FeatureValue(ValueDefinition("feat3", "feat"), 5)
    reading.set(feature3, level3)

    dtg = DataTrace.from_reading(reading)

    level1 = reading.get_level("level_1")
    raw_data_set1 = level1.get_raw_data_set("data-set-1")
    feature1 = FeatureValue(ValueDefinition("feat1", "feat"), 3)

    reading.set(feature1, level1)
    dtg.add_trace(raw_data_set1, feature1, EXTRACT_STEP)
...
def test_data_trace_consistency_check(reading_example):
    ...
    dtg = DataTrace.from_reading(reading_example)
    warning_match = "No feature has been created from the following entity"
    ...
    for id_ in ("level_1", "level_2"):
        ...
        feature1 = FeatureValue(ValueDefinition(f"{id_}_feat1", "feat"), 3)
        feature2 = FeatureValue(ValueDefinition(f"{id_}_feat2", "feat"), 3)
        dtg.add_trace(raw_data_set1, feature1, EXTRACT_STEP)
        dtg.add_trace(raw_data_set2, feature2, EXTRACT_STEP)

    with pytest.warns(None, match=warning_match) as record:
        dtg.check_data_set_usage()
    assert len(record) == 0

    dtg.add_trace(feature1, Level(id_="id", start=0, end=1), ProcessingStep())
    with pytest.raises(ValueError):
        dtg.check_data_set_usage()


def test_data_trace_get_feature_flags(reading_example):
    """Test data trace feature flag retrieval."""
    reading = deepcopy(reading_example)
    feature = FeatureValue(ValueDefinition("feat2", "feat"), 5)
    feature_set = reading.get_level("level_1").feature_set
    feature_set.set(feature)
    feature_set.get("feat2").add_flag(Flag("pinch-technical-deviation-ta", "reason 1"))
    dtg = DataTrace.from_reading(reading)
    assert dtg.get_feature_flags() == [Flag("pinch-technical-deviation-ta", "reason 1")]


def test_data_trace_add_feature_level_outcome_node(data_trace_example):
    """Test adding trace between a feature towards a level."""
    dtg = deepcopy(data_trace_example)
    reading = dtg.get_reading()
    level_1 = reading.get_level("level_1")
    level_2 = reading.get_level("level_2")
    feat_1 = level_1.feature_set.get("feat1")
    step = ProcessingStep()
